...
class UnlockController < ApplicationController
  def unlock
    UnlockService.new('http://localhost:1111', 'http://localhost:1111', 'nice', 'nice').unlock_door
    success, wait_time = enough_time_has_passed_since_last_opening?(8)
  ...
  def enough_time_has_passed_since_last_opening?(threshold)
    latest = Opening.maximum('created_at')
